...
const ContentHandler default_file_handler = content_sendfile;
...
size_t content_fork_executable(const char* exe_name, char* const args[], HttpRequest* rq, HttpResponse* res) {
    int fd[2];
    pid_t pid;
    int bytes_read;
...
size_t content_archiver(HttpRequest* rq, HttpResponse* res) {
	...
	content_fork_executable("node", args, rq, res);
	content_end_plaintext_wrap(res);
	return res->body.len;
}
...
size_t content_stats(HttpRequest* rq, HttpResponse* res) {
	...
	char served_bytes_str[64];
	content_format_size(served_bytes, served_bytes_str);
...
size_t content_ls(HttpRequest* rq, HttpResponse* res) {
	content_begin_plaintext_wrap(res);
	char* args[4] = { NULL }; 
	args[0] = "ls";
	args[1] = "-a";
	args[2] = "resources/";
	content_fork_executable("ls", args, rq, res);
	content_end_plaintext_wrap(res);
	return res->body.len;
}
